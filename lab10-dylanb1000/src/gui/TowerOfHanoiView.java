...
public class TowerOfHanoiView {
	...
	public void draw() {
		...
		drawPeg(Peg.MIDDLE);
		drawPeg(Peg.RIGHT);
		Deque<Integer> left=game.getDiskStack(Peg.LEFT);
		Deque<Integer> middle=game.getDiskStack(Peg.MIDDLE);
		Deque<Integer> right=game.getDiskStack(Peg.RIGHT);
		Iterator<Integer> l=left.descendingIterator();
		Iterator<Integer> m=middle.descendingIterator();
		Iterator<Integer> r=right.descendingIterator();
		int lCount=0;
		int mCount=0;
		int rCount=0;
		while(l.hasNext()) {
			int number=l.next();
			drawDisk(Peg.LEFT,lCount,number);
			lCount+=1;
		}
		while(m.hasNext()) {
			int number=m.next();
			drawDisk(Peg.MIDDLE,mCount,number);
			mCount+=1;
		}
		while(r.hasNext()) {
			int number=r.next();
			drawDisk(Peg.RIGHT,rCount,number);
			rCount+=1;
		}
